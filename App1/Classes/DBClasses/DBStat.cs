...
namespace App1.Classes.DBClasses
{
    public class DBStat
    {
        ...
        public DBStat(int id, int nidseason, int nidmatch, int nidposition, int nnumber, int nidplayer, int niduserteam,
            int nminutes, int ngoals, int nassists, int npenalties, int nredcards, int nplusminus, string scomment, DateTime dinserted, DateTime dupdated)
        {
            this.nID = id;
            ...
            try
            {
                ...
                while (query.Read())
                {
                    var item = new DBStat
                    {
                        ...
                        sComment = query.GetString(query.GetOrdinal("sComment")),
        ...
        public void ChangeDB(string action)
        {
            ...
            {
                case "add":
                    sCommand = "INSERT INTO tbl_stats (nID, nIDSeason, nIDMatch, nIDPosition, nNumber, nIDPlayer, nIDUserTeam, nMinutes, nGoals, nAssistance, nPenalties, nRedCards, nPlusMinus, sComment, dInserted, dUpdated)" +
                                        " VALUES('" + nID + "', " +
                                        "'" + nIDSeason + "', " +
                                        "'" + nIDMatch + "', " +
                                        "'" + nIDPosition + "', " +
